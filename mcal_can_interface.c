...
void App_CanInit(void)
{
    ...
    //<<CAN for 250 kbps
    stcCanInitCfg.stcCanBt.PRESC = 2 - 1;     // Divide by 2
    stcCanInitCfg.stcCanBt.SEG_1 = 12 - 2;    // SEG_1 = 10
    stcCanInitCfg.stcCanBt.SEG_2 = 4 - 1;     // SEG_2 = 3
...
App_CanFrameFromQueue(can_frame_info_t *pkt)  //   App_CanFrameFromQueue(&std_can_pdu);  call this in every 10ms;viz timer
{
    ...
    //App_CanMessageHook(pkt);
    CAN_ID =pkt->u32_can_id;
    for(uint8_t i=0; i<= pkt->u8_dlc; i++)
    {
     candata[i] = pkt->u8_data[i];
    }

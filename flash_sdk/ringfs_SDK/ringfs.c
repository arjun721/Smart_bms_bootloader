...
{
    int retSectorStatus = 0;
    int sector_addr = _sector_address(fs, sector);
    retSectorStatus = _sector_set_status(fs, sector, SECTOR_ERASING);
    if(retSectorStatus == -1){
            return -1;
    }

    if(fs->flash->sector_erase != NULL){
    ...
    if(fs->flash->program != NULL){
        retSectorStatus = fs->flash->program(fs->flash,
    ...
    retSectorStatus = _sector_set_status(fs, sector, SECTOR_FREE);
    if(retSectorStatus == -1){
            return -1;
    }
    return 0;
...
int ringfs_format(struct ringfs *fs, int8_t (*wdg_refresh)())
{
    ...
    for (int sector=0; sector<fs->flash->sector_count; sector++){
        int8_t wdg = 0;//(*wdg_refresh)();
        retSectorStatus = 0;
        retSectorStatus = _sector_set_status(fs, sector, SECTOR_FORMATTING);
        if(retSectorStatus == -1){
            return -1;
        }
    }
    ...
    for (int sector=0; sector<fs->flash->sector_count; sector++){
        int8_t wdg = 0;//(*wdg_refresh)();
        retSectorStatus = 0;
        retSectorStatus = _sector_free(fs, sector);
        if(retSectorStatus == -1){
            return -1;
        }
    }
...
int ringfs_scan(struct ringfs *fs)
{
    ...
    for (int sector=0; sector<fs->flash->sector_count; sector++) {
        ...
        if(fs->flash->read != NULL){
            retSectorStatus = fs->flash->read(fs->flash, addr, &header, sizeof(header));
        }
        else{
            return -1;
        }
...
//            printf("ringfs_scan: partially formatted partition\r\n");
        ...
        if (header.status == SECTOR_ERASING || header.status == SECTOR_ERASED) {
            ...
            // if(header.status == SECTOR_ERASING){
...
//        printf("ringfs_scan: invariant violated: no FREE sector found\r\n");
    ...
    while (fs->write.sector == write_sector) {
        int slotRetStatus = 0;
        uint32_t status;
        slotRetStatus = _slot_get_status(fs, &fs->write, &status);
        ...
        if(status == SLOT_RESERVED){
            slotRetStatus = _slot_set_status(fs, &fs->write, SLOT_GARBAGE);
            if(slotRetStatus == -1){
                return -1;
            }
        }
    ...
    while (!_loc_equal(&fs->read, &fs->write)) {
        uint32_t status;
        int slotRetStatus = 0;
        slotRetStatus = _slot_get_status(fs, &fs->read, &status);
        if(slotRetStatus == -1){
            return -1;
        }
...
int ringfs_count_exact(struct ringfs *fs)
{
    ...
    while (!_loc_equal(&loc, &fs->write)) {
        uint32_t status;
        int slotRetStat = 0;
        slotRetStat = _slot_get_status(fs, &loc, &status);
        if(slotRetStat == -1){
            return -1;
        }
        __asm("nop");
...
int ringfs_append(struct ringfs *fs, const void *object)
{
    ...
    int retSectorStatus = 0;
    retSectorStatus = _sector_get_status(fs, next_sector, &status);
    if(retSectorStatus == -1){
        return -1;
    }
    if (status != SECTOR_FREE) {
        ...
        retSectorStatus = _sector_free(fs, next_sector);
        if(retSectorStatus == -1){
            return -1;
        }
    }
    ...
    retSectorStatus = _sector_get_status(fs, fs->write.sector, &status);
    if(retSectorStatus == -1){
        return -1;
    }
    if (status == SECTOR_FREE) {
        /* Free sector. Mark as used. */
        retSectorStatus = _sector_set_status(fs, fs->write.sector, SECTOR_IN_USE);
        if (retSectorStatus == -1){
            return -1;
        }
...
//        printf("ringfs_append: corrupted filesystem\r\n");
    ...
    /* Preallocate slot. */
    writeStatus = _slot_set_status(fs, &fs->write, SLOT_RESERVED);
    if(writeStatus == -1){
        return -1;
    }
    ...
    /* Write object. */
    if(fs->flash->program != NULL){
        writeStatus = fs->flash->program(fs->flash,
    ...
    /* Commit write. */
    writeStatus = _slot_set_status(fs, &fs->write, SLOT_VALID);
    if(writeStatus == -1){
        __asm("nop");
        return -1;
    }
...
int ringfs_fetch(struct ringfs *fs, void *object)
{
    ...
    while (!_loc_equal(&fs->cursor, &fs->write)) {
        ...
        slotRetStatus = _slot_get_status(fs, &fs->cursor, &status);
        if(slotRetStatus == -1){
            return -1;
        }
        ...
        if (status == SLOT_VALID) {
            int readStatus = -1;
            if(fs->flash->read != NULL){
                readStatus = fs->flash->read(fs->flash,
...
int ringfs_discard(struct ringfs *fs)
{
    ...
    while (!_loc_equal(&fs->read, &fs->cursor)) {
        retSlotStatus = _slot_set_status(fs, &fs->read, SLOT_GARBAGE);
        if(retSlotStatus == -1){
            return -1;
        }
        _loc_advance_slot(fs, &fs->read);
...
/**
 * @brief return total logs from current read cursor and write cursor, 
 *        latestValidSlot data pointer contains log number of latest 
 *        valid slot out of all the slots
 * 
 * @param fs 
 * @param latestValidSlot 
 * @return int 
 */
int ringfs_getTotalSlotsandLatesValidSlot(struct ringfs *fs, uint32_t * latestValidSlot)
{
    ...
    *latestValidSlot = validSlotNum;
    __asm("nop");
    return slots;
}


/** Advance the cursor to a specific location, advancing the sector too if needed*/
int ringfs_advance_slot_specific(struct ringfs *fs, uint32_t moveToLogNum)
{
	uint32_t ijk = 1;
	for(ijk = 1; ijk <= moveToLogNum; ijk++){
		_loc_advance_slot(fs, &fs->cursor);
	}
	return (ijk-1);
}
...
{
    if(fs->eraseCursor >= fs->flash->sector_count)
    {
        return 1;
    }
...
/* Erase, update version, mark as free. */
    ...
    int8_t wdg = 0;//(*wdg_refresh)();
    int retSectorStatus = 0;
    retSectorStatus = _sector_free(fs, fs->eraseCursor);            //TODO: add check "fs->eraseCursor" should be less than "fs->flash->sector_count"
    if(retSectorStatus == -1){
        return 1;
    }
    #if SEV_SEG_DB == 1
    ...
    #endif
    uint32_t status;
    retSectorStatus = _sector_get_status(fs, fs->eraseCursor, &status);
    if(retSectorStatus == -1){
        return 1;
    }
    if(status != SECTOR_FREE){
...
{
    int checkkk = 0,sectoradd = 0,slotadd = 0;
    sectoradd = _sector_address(fs, AvailableSector);
    slotadd = _slot_address(fs, &fs->write) + sizeof(struct slot_header) + objSize;
    checkkk = slotadd < sectoradd;
    if(checkkk)
    {
        __asm("nop");
        return 1;
